...
# Add: Visualization of GT for a few batches during train
def visualize_gt_batch(samples, targets, vis_dir, max_visualizations=3):
    """
    Save GT annotation visualization for up to max_visualizations images in the batch.
    Only ground-truth is drawn (red dots).
    """
    ...
    images = samples.tensors
    masks = samples.mask
    num_to_save = min(images.shape[0], max_visualizations)
    for idx in range(num_to_save):
        ...
        name = f"batch{idx+1}"
        cv2.imwrite(os.path.join(vis_dir, '{}_GTONLY.jpg'.format(name)), sample_vis)
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    # To limit excessive GT visualization, visualize only for first N batches per epoch
    # gt_vis_batch_limit = 30
    # gt_vis_count = 0
    # gt_vis_dir = "./vis_train_gt"
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        samples = samples.to(device)
        targets = [{k: v.to(device) for k, v in t.items()} for t in targets]
        gt_points = [target['points'] for target in targets]

        # Draw GT annotations for a few images each epoch for inspection
        # if gt_vis_count < gt_vis_batch_limit:
        #     # Save images in a dedicated directory, one-time per batch
        #     visualize_gt_batch(samples, targets, gt_vis_dir)
        #     gt_vis_count += 1
...
def evaluate(model, data_loader, device, epoch=0, vis_dir=None):
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        with torch.inference_mode():
            with torch.cuda.amp.autocast(dtype=torch.float16):  # 혹은 bfloat16 지원 시 torch.bfloat16
        ...
        # print(f"predict_cnt: {predict_cnt}, gt_cnt: {gt_cnt}")
        # compute error
